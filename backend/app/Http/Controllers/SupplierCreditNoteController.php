...
class SupplierCreditNoteController extends Controller{
    ...
    public function getAllSupplierCreditNote(Request $request){
        $perPage = (int) $request->input('per_page', 15);
        $page = (int) $request->input('page', 1);
        $search = $request->input('search', '');
        $customerCodes = $request->input('supplier_codes', []);
        $dateFrom = $request->input('date_from');
        $dateTo = $request->input('date_to');
        $categoryDates = $request->input('category_dates');

        // Base query with eager loading
        $query = Credit_note_supplier_master::with([
            ...
            'details.chartOfAccount:id,account_code,account_name_en,account_name_cn',
            'invoiceStatus:id,status_value_en,status_value_cn',
        ])->orderByDesc('id');
        
        // Filters
        if (!empty($customerCodes)) {
            $query->whereIn('supplier_id', $customerCodes);
        }
    ...
    public function getSuppCRInfo($id){
        $list = Credit_note_supplier_master::with([
            'supplier',
            'details.product',
        ...
        $suppliername_en = $list->supplier->suppliername_en ?? '';
        $suppliername_cn = $list->supplier->suppliername_cn ?? '';
        $suppliername_cn = $suppliername_cn === '' ? $suppliername_en : $suppliername_cn;

        $supplier_address_en = $list->supplier->supplier_address_en ?? '';
        $supplier_address_cn = $list->supplier->supplier_address_cn ?? '';
        ...
        $mergedDetailsModels = $list->details;
        ...
        $list = [
            ...
            'operator'              => $operator,
            'particulars_en'        => $list->remarks_en,
            'particulars_cn'        => $list->remarks_cn,
            'amount'                => $list->amount,
    ...
    private function transformOrderData($list){
        ...
        return [
            ...
            'suppliername_en' => $suppliername_en,
            'suppliername_cn' => $suppliername_cn,
            'remarks_en' => $list->remarks_en,
            'remarks_cn' => $list->remarks_cn,
            'details' => $details->map(function ($detail) {
                $product = optional($detail->product);
                $account = optional($detail->chartOfAccount);
    ...
    public function voidSupplierCreditNote(Request $request){
        DB::beginTransaction();
        ...
        try {
            ...
            foreach ($request->details as $list) {
                ...
                $cr = Credit_note_supplier_master::where('cr_number', $cr_number)->first();
                if ($cr) {
                    $cr->cr_status_id = 5;
    ...
    public function updateSuppCreditNote(Request $request, $id){
        ...
        try {
            ...
            if ((int) $id === 0) {
                ...
                $cr_number = $AutoGenerated->getNextNo("cr_number","CR","t_credit_note_supplier_master");
                $inserMaster = [
                    'cr_number' => $cr_number,
                    ...
                    'remarks_en' => $request->particulars_en,
                    'remarks_cn' => $request->particulars_cn,
                ];
                $master = Credit_note_supplier_master::create($inserMaster);
                $OrigID = $master->id;
                $GlobalTableNo = $cr_number;
            }
            else{
                $master = Credit_note_supplier_master::find($request['id']);
                if ($master) {
                    ...
                    $master->remarks_en = $request->particulars_en;
                    $master->remarks_cn = $request->particulars_cn;
                    $master->save();
                }
            ...
            if ($request->has('details')) {
                foreach ($request->details as $list) {
                    ...
                    if ((int) $list['is_deleted'] === 1) {
                        $detail = Credit_note_supplier_detail::find($recordID);
                        if ($detail) {
                            $detail->delete();
                        }
                    }
                    if((int) $list['is_deleted'] === 0){
                        if ((int) $recordID === 0) {
                            ...
                            $detail = Credit_note_supplier_detail::create($insertDetail);
                            $detail_id = $detail->id;
                        }
                        else{
                            $detail = Credit_note_supplier_detail::find($recordID);
            ...
            $globalController = new GlobalController();
            $globalController->logAction(
                'Credit Note (Supplier)', 't_credit_note_supplier_master',
                $Action,
