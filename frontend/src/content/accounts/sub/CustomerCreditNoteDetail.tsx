...
import { customerCreditNoteService, ApiCreditNote, DetailsRow } from "@/services/customerCreditNoteService";
...
interface ReceiveVoucherDetailsProps {
    ...
    onSave: () => void;
    tabId: string;
    onChangeCustomerCreditNoteId: (newId: number) => void;
...
const ReceiveVoucherDetails: React.FC<ReceiveVoucherDetailsProps> = ({ creditNoteId, tabId, saveType, onBack, onSave, onChangeCustomerCreditNoteId, onChangeSaveType }) => {
    ...
    const fetchCreditNote = async () => {
        try {
            ...
            if (cachedSuppliersRaw) {
                ...
                foundSupplier = await customerCreditNoteService.getCRInfo(idToUse); // ✅ Fetch from API
            }
            if (foundSupplier) {
    ...
    const fetchChartsOfAccounts = async (page = currentPageAccounts, perPage = itemsPerPageAccounts, search = "") => {
        try {
            ...
            const paginatedData = await customerCreditNoteService.getChartsOfAccount(page, perPage, search, "");
    ...
    const updateCachedRaw = async (cachedType: string, masterId: number) => {
        if (cachedType === "info") {
            const newData = await customerCreditNoteService.getCRInfo(masterId);
            if (newData) {
    ...
    const handleSave = async () => {
        ...
        try {
            const result = await customerCreditNoteService.updateCreditNote(creditNoteId, data);
            ...
            if ((saveType === "new" || saveType === "copy") && newId) {
                onChangeCustomerCreditNoteId(newId);
    ...
    const handleAddNew = async () => {
        onChangeCustomerCreditNoteId(0);
        ...
        switch (column) {
            case "account_code":
                try {
                    ...
                    const res = await customerCreditNoteService.getChartsOfAccount(currentPageAccounts, itemsPerPageAccounts, "", value);
                ...
                break;
            case "customer_code":
                const res = await customerCreditNoteService.getCustomerInfoByCode(value, "enter");
    ...
    const handleVoid = async () => {
        ...
        if (!confirmed) return;
        try {
            const result = await customerCreditNoteService.voidCustomerCreditNote(data);
            if (result.token === "Error") {
                showErrorToast(result.message);
                return;
            }
    ...
    const handleRowClick_Customer = async (e: React.MouseEvent, tableId: number) => {
        ...
        const res = await customerCreditNoteService.getCustomerInfoByCode(String(customer_code), "click");
        if (res.token === "CustomerNotExists") {
    ...
    return (
        <div className="h-screen flex flex-col" style={{ backgroundColor: "#1a1a1a" }}>
            ...
            <div className="hidden">
                <ExportReportSelector ref={exportRef} formats={["odt"]} baseName="CRCustomer_Report" ids={[creditNoteId]} language={lang} />
            </div>
