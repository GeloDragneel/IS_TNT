...
import { customerCreditNoteService, ApiCreditNote, DetailsRow, FilterParams } from "@/services/customerCreditNoteService";
...
interface ReceiveVoucherListProps {
    tabId: string;
    onCustomerCreditNoteSelect: (creditNoteId: number, saveType?: string) => void;
...
// localStorage.clear();
const ReceiveVoucherList: React.FC<ReceiveVoucherListProps> = ({ tabId, onCustomerCreditNoteSelect, selectedCR, onSelectedCRChange }) => {
    ...
    const fetchCreditNote = async (page = currentPage, perPage = itemsPerPage, filters: FilterParams = { search: "" }) => {
        try {
            ...
            const paginatedData = await customerCreditNoteService.getAllCustomerCreditNote(page, perPage, filters);
    ...
    const handleRowClick = (e: React.MouseEvent, creditNoteId: number) => {
        ...
        onCustomerCreditNoteSelect(creditNoteId, creditNoteId == 0 ? "new" : "edit");
    };
                        ...
                        <table className="w-full">
                            <thead className="sticky top-0 z-[1]" style={{ backgroundColor: "#1f2132" }}>
                                <tr className="border-b" style={{ borderColor: "#2d2d30" }}>
                                    ...
                                    <th className="w-[25%] text-left py-2 px-2 text-gray-400 text-sm">{translations["Customer"]}</th>
                ...
                <div className="border-t flex items-center justify-between" style={{ borderColor: "#404040" }}>
                    <div className="p-4 flex items-center space-x-1">
                        ...
                        <ExportReportSelector
                            ...
                            baseName="CustomerCreditNote"
                            ids={selectedCR.length > 0 ? selectedCR : creditNoteList.map((p) => p.id)}
                            language={lang}
                        />
                    </div>
