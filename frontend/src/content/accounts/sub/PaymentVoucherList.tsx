...
interface PaymentVoucherListProps {
    tabId: string;
    onPaymentVoucherSelect: (paymentVoucherId: number, saveType?: string) => void;
    onChangeView: (view: "list" | "details") => void;
    selectedPV: number[];
    onSelectedPVChange: (selected: number[]) => void;
    expandedRows: number[];
...
// localStorage.clear();
const PaymentVoucherList: React.FC<PaymentVoucherListProps> = ({ tabId, onPaymentVoucherSelect, selectedPV, onSelectedPVChange }) => {
    const { translations, lang } = useLanguage();
    const [pvoucherList, setPaymentVoucher] = useState<ApiPaymentVoucher[]>([]);
    ...
    const [loading, setLoading] = useState(() => {
        const savedLoading = localStorage.getItem(`${tabId}-loading-pv-list`);
    ...
    const [currentPage, setCurrentPage] = useState(() => {
        const metaKey = `${tabId}-cached-meta-pv-list`;
    ...
    const [searchTerm, setSearchTerm] = useState(() => {
        const cached = localStorage.getItem(`${tabId}-cached-meta-pv-list`);
        if (!cached) return "";
        try {
    ...
    useEffect(() => {
        ...
        if (cachedExpandedKey) {
            ...
            parseExpanded.forEach((tableId: number) => {
                ...
                const data = pvoucherList.find((po) => po.id === tableId);
                // Save to state
                if (!data?.details) {
    ...
    useEffect(() => {
        ...
        channel.listen(".pv-event", () => {
            ...
            localStorage.removeItem(`${tabId}-cached-pv-list`);
            setTimeout(() => {
                fetchPaymentVoucher(currentPage, itemsPerPage, defaultFilters);
            }, 500);
        });
        return () => {
    ...
    // ON LOAD LIST
    useEffect(() => {
        const MasterKey = `${tabId}-cached-pv-list`;
        const metaKey = `${tabId}-cached-meta-pv-list`;
        ...
        try {
            ...
            if (isCacheValid) {
                setPaymentVoucher(cachedSuppliers);
                setTotalPages(cachedMeta.totalPages);
                setLoading(false);
                localStorage.setItem(`${tabId}-loading-pv-list`, JSON.stringify(false));
                return;
            }
        } catch (err) {
            console.warn("Failed to parse cached meta:", err);
    ...
    const fetchPaymentVoucher = async (page = currentPage, perPage = itemsPerPage, filters: FilterParams = { search: "" }) => {
        try {
            if (loading) {
                setLoading(true);
            }
            localStorage.setItem(`${tabId}-loading-pv-list`, JSON.stringify(true));
            const paginatedData = await paymentVoucherService.getAllPVList(page, perPage, filters);
            setPaymentVoucher(paginatedData.data);
            setTotalPages(paginatedData.last_page);
            setPVFooter2(paginatedData.footer2);
            localStorage.setItem(`${tabId}-cached-pv-list`, JSON.stringify(paginatedData.data));
            localStorage.setItem(
                `${tabId}-cached-meta-pv-list`,
                JSON.stringify({
                    page,
                    perPage,
                    filters,
    ...
    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            onSelectedPVChange(pvoucherList.map((p) => p.id).filter(Boolean) as number[]);
        } else {
            onSelectedPVChange([]);
        }
    };
    ...
    const handleDateRange = () => {
        ...
        fetchPaymentVoucher(1, itemsPerPage, filters);
    };
    const handleReset = () => {
        setFormData({
            ...formData,
    ...
    const handleSelectInv = (id: number, checked: boolean) => {
        const updated = checked ? [...selectedPV, id] : selectedPV.filter((pid) => pid !== id);

        onSelectedPVChange(updated);
    };
    const handleVoidSelected = async () => {
        ...
        const selectedPVMaster = pvoucherList.filter((item) => selectedPV.includes(item.id));
        const data = new FormData();
        selectedPVMaster.forEach((list: any) => {
            if (list) {
    ...
    const handleRowClick = (e: React.MouseEvent, paymentVoucherId: number) => {
        ...
        onPaymentVoucherSelect(paymentVoucherId, paymentVoucherId == 0 ? "new" : "edit");
    };
            ...
            <div className="rounded-lg border shadow-sm" style={{ backgroundColor: "#19191c", borderColor: "#404040" }}>
                                ...
                                <tr className="border-b" style={{ borderColor: "#2d2d30" }}>
                                    ...
                                    <th className="w-[3%] text-center py-2 px-2 text-gray-400 text-sm">
                                        <div className="flex justify-center items-center">
                                            <CustomCheckbox checked={selectedPV.length === pvoucherList.length && pvoucherList.length > 0} onChange={(checked) => handleSelectAll(checked)} />
                                        </div>
                                    </th>
                                            ...
                                            <React.Fragment key={list.id || index}>
                                                <tr
                                                    ...
                                                    className={`clickable border-b hover:bg-gray-700 hover:bg-opacity-30 transition-colors cursor-pointer ${
                                                        selectedPV.includes(list.id as number) ? "bg-gray-700 bg-opacity-30" : ""
                                                    }`}
                                                ...
                                                >
                                                    ...
                                                    <td className="py-2 px-2 text-gray-400 text-center text-custom-sm" onClick={(e) => e.stopPropagation()}>
                                                        <CustomCheckbox
                                                            ...
                                                            checked={selectedPV.includes(list.id as number)}
                ...
                <div className="border-t flex items-center justify-between" style={{ borderColor: "#404040" }}>
                    <div className="p-2 flex items-center space-x-1">
                        ...
                        />
                        <ExportReportSelector formats={["odt", "ods", "xlsx"]} baseName="PaymentVoucher" ids={selectedPV.length > 0 ? selectedPV : pvoucherList.map((p) => p.id)} language={lang} />
                        <div className="hidden">
                            <ExportReportSelector ref={exportRef} formats={["odt"]} baseName="DownloadSelectedSinglePV" ids={selectedPV} language={lang} />
                        </div>
                    </div>
