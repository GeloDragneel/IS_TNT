...
import { receiveVoucherService, ApiReceiveVoucher, DetailsRow, FilterParams } from "@/services/receiveVoucherService";
...
interface ReceiveVoucherListProps {
    tabId: string;
    onReceiveVoucherSelect: (receiveVoucherId: number, saveType?: string) => void;
    onChangeView: (view: "list" | "details") => void;
    selectedRV: number[];
    onSelectedRVChange: (selected: number[]) => void;
    expandedRows: number[];
...
// localStorage.clear();
const ReceiveVoucherList: React.FC<ReceiveVoucherListProps> = ({ tabId, onReceiveVoucherSelect, selectedRV, onSelectedRVChange }) => {
    const { translations, lang } = useLanguage();
    const [rvoucherList, setReceiveVoucher] = useState<ApiReceiveVoucher[]>([]);
    const [totalPages, setTotalPages] = useState(1);
    const locale = getFlatpickrLocale(translations);
    const pageSizeOptions = useMemo(() => [15, 20, 50, -1], []);
    const [expanded_RV, setExpandedRows] = useState<number[]>([]);
    ...
    const [rvFooter2, setRVFooter2] = useState<POFooter[]>([]); // Changed to array
    const inputRef = useRef<HTMLInputElement>(null);
    const [loading, setLoading] = useState(() => {
        const savedLoading = localStorage.getItem(`${tabId}-loading-rv-list`);
    ...
    const [currentPage, setCurrentPage] = useState(() => {
        const metaKey = `${tabId}-cached-meta-rv-list`;
    ...
    const [searchTerm, setSearchTerm] = useState(() => {
        const cached = localStorage.getItem(`${tabId}-cached-meta-rv-list`);
        if (!cached) return "";
        try {
    ...
    useEffect(() => {
        ...
        if (cachedExpandedKey) {
            ...
            parseExpanded.forEach((tableId: number) => {
                ...
                const data = rvoucherList.find((po) => po.id === tableId);
                // Save to state
                if (!data?.details) {
    ...
    useEffect(() => {
        ...
        channel.listen(".rv-event", () => {
            ...
            localStorage.removeItem(`${tabId}-cached-rv-list`);
            setTimeout(() => {
                fetchReceiveVoucher(currentPage, itemsPerPage, defaultFilters);
            }, 500);
        });
        return () => {
    ...
    // ON LOAD LIST
    useEffect(() => {
        const MasterKey = `${tabId}-cached-rv-list`;
        const metaKey = `${tabId}-cached-meta-rv-list`;
        ...
        try {
            ...
            if (isCacheValid) {
                setReceiveVoucher(cachedSuppliers);
                setTotalPages(cachedMeta.totalPages);
                setLoading(false);
                localStorage.setItem(`${tabId}-loading-rv-list`, JSON.stringify(false));
                return;
            }
        } catch (err) {
            console.warn("Failed to parse cached meta:", err);
    ...
    const fetchReceiveVoucher = async (page = currentPage, perPage = itemsPerPage, filters: FilterParams = { search: "" }) => {
        try {
            if (loading) {
                setLoading(true);
            }
            localStorage.setItem(`${tabId}-loading-rv-list`, JSON.stringify(true));
            const paginatedData = await receiveVoucherService.getAllRVList(page, perPage, filters);
            setReceiveVoucher(paginatedData.data);
            setTotalPages(paginatedData.last_page);
            setRVFooter2(paginatedData.footer2);
            localStorage.setItem(`${tabId}-cached-rv-list`, JSON.stringify(paginatedData.data));
            localStorage.setItem(
                `${tabId}-cached-meta-rv-list`,
                JSON.stringify({
                    page,
                    perPage,
                    filters,
    ...
    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            onSelectedRVChange(rvoucherList.map((p) => p.id).filter(Boolean) as number[]);
        } else {
            onSelectedRVChange([]);
        }
    };
    ...
    const handleDateRange = () => {
        ...
        fetchReceiveVoucher(1, itemsPerPage, filters);
    };
    const handleReset = () => {
        setFormData({
            ...formData,
    ...
    const handleSelectInv = (id: number, checked: boolean) => {
        const updated = checked ? [...selectedRV, id] : selectedRV.filter((pid) => pid !== id);

        onSelectedRVChange(updated);
    };
    const handleVoidSelected = async () => {
        if (selectedRV.length === 0) {
            showErrorToast(translations["Please click checkbox to select row"]);
            return;
        }
        const selectedRVMaster = rvoucherList.filter((item) => selectedRV.includes(item.id));
        const data = new FormData();
        selectedRVMaster.forEach((list: any) => {
            if (list) {
                data.append("details[]", JSON.stringify({ rv_number: list.rv_number }));
            }
        });
    ...
    const handleRowClick = (e: React.MouseEvent, receiveVoucherId: number) => {
        ...
        onReceiveVoucherSelect(receiveVoucherId, receiveVoucherId == 0 ? "new" : "edit");
    };
    ...
    const handleToggleRow = async (tableId: number) => {
        const isExpanded = expanded_RV.includes(tableId);
        const expandedKey = tabId + "_expandedKey";

        if (isExpanded) {
            // Collapse row
            const updated = expanded_RV.filter((id) => id !== tableId);
            setExpandedRows(updated);
            localStorage.setItem(expandedKey, JSON.stringify(updated));
        } else {
            // Expand row
            const updated = [...expanded_RV, tableId];
            setExpandedRows(updated);
            localStorage.setItem(expandedKey, JSON.stringify(updated));
            try {
                const data = rvoucherList.find((po) => po.id === tableId);
            ...
            <div className="rounded-lg border shadow-sm" style={{ backgroundColor: "#19191c", borderColor: "#404040" }}>
                                ...
                                <tr className="border-b" style={{ borderColor: "#2d2d30" }}>
                                    ...
                                    <th className="w-[3%] text-center py-2 px-2 text-gray-400 text-sm">
                                        <div className="flex justify-center items-center">
                                            <CustomCheckbox checked={selectedRV.length === rvoucherList.length && rvoucherList.length > 0} onChange={(checked) => handleSelectAll(checked)} />
                                        </div>
                                    </th>
                                    <th className="w-[8%] text-left py-2 px-2 text-gray-400 text-sm">{translations["RV Date"]}</th>
                                    <th className="w-[15%] text-left py-2 px-2 text-gray-400 text-sm">{translations["Customer"]}</th>
                                    <th className="w-[15%] text-left py-2 px-2 text-gray-400 text-sm">{translations["Transaction Details"]}</th>
                                    <th className="w-[13%] text-left py-2 px-2 text-gray-400 text-sm">{translations["RV No"]}</th>
                                    <th className="w-[10%] text-center py-2 px-2 text-gray-400 text-sm">{translations["Ex Rate"]}</th>
                                    <th className="w-[11%] text-left py-2 px-2 text-gray-400 text-sm">{translations["Bank"]}</th>
                                    <th className="w-[11%] text-center py-2 px-2 text-gray-400 text-sm">{translations["AmountPaid2"]}</th>
                                    <th className="w-[11%] text-center py-2 px-2 text-gray-400 text-sm">{translations["RV Status"]}</th>
                                </tr>
                            </thead>
                            {loading ? (
                                            ...
                                            <React.Fragment key={list.id || index}>
                                                <tr
                                                    ...
                                                    className={`clickable border-b hover:bg-gray-700 hover:bg-opacity-30 transition-colors cursor-pointer ${
                                                        selectedRV.includes(list.id as number) ? "bg-gray-700 bg-opacity-30" : ""
                                                    }`}
                                                ...
                                                >
                                                    <td className="py-2 px-2 text-gray-400 text-left text-custom-sm" onClick={(e) => e.stopPropagation()}>
                                                        <button
                                                            ...
                                                            className={`px-1 py-1 ${
                                                                expanded_RV.includes(list.id) ? "bg-red-600 hover:bg-red-700" : "bg-green-600 hover:bg-green-700"
                                                            } text-white rounded-lg transition-colors text-xs`}
                                                        >
                                                            {expanded_RV.includes(list.id) ? <Minus size={16} /> : <Plus size={16} />}
                                                        </button>
                                                    </td>
                                                    <td className="py-2 px-2 text-gray-400 text-center text-custom-sm" onClick={(e) => e.stopPropagation()}>
                                                        <CustomCheckbox
                                                            ...
                                                            checked={selectedRV.includes(list.id as number)}
                                                            onChange={(checked) => handleSelectInv(list.id as number, checked)}
                                                        />
                                                    </td>
                                                    <td className="py-2 px-2 text-gray-400 text-left text-custom-sm">{formatDate(list.rv_date, lang)}</td>
                                                    ...
                                                    <td className="py-2 px-2 text-gray-400 text-left text-custom-sm">
                                                        {list.count > 1 ? (
                                                            <p>{lang === "en" ? translations[list.trans_detail_en || ""] : translations[list.trans_detail_cn || ""]}</p>
                                                        ) : (
                                                            <div className="flex flex-col space-y-1">
                                                                <div className="group flex items-center">
                                                                    <p className="text-gray-400 text-custom-sm">{lang === "en" ? list.trans_detail_en : list.trans_detail_cn}</p>
                                                                    <CopyToClipboard text={lang === "en" ? list.trans_detail_en || "" : list.trans_detail_cn || ""} />
                                                                </div>
                                                            </div>
                                                        )}
                                                    </td>
                                                    <td className="py-2 px-2 text-gray-400 text-left text-custom-sm">
                                                        <div className="group flex items-center">
                                                            <p className="text-gray-400 text-custom-sm">{highlightMatch(list.rv_number, searchTerm)}</p>
                                                            <CopyToClipboard text={list.rv_number ?? ""} />
                                                        </div>
                                                    </td>

                                                    <td className="py-2 px-2 text-gray-400 text-center text-custom-sm">{formatMoney(list.ex_rate)}</td>
                                                    <td className="py-2 px-2 text-gray-400 text-left text-custom-sm">{lang === "en" ? list.bank_name_en : list.bank_name_cn}</td>
                                                    ...
                                                    <td className="py-2 px-2 text-gray-400 text-center text-custom-sm">
                                                        <span className={`px-2 py-1 rounded-full text-xs text-custom-sm ${getStatusColor(list.rv_status_id)}`}>
                                                            {lang === "en" ? list.status_value_en : list.status_value_cn}
                                                        </span>
                                                    </td>
                                                </tr>
                                                {expanded_RV.includes(list.id) && (
                                                                        ...
                                                                        {soDetailMap[list.id] && soDetailMap[list.id]!.length > 0 ? (
                                                                            soDetailMap[list.id]!.map((detail, i) => (
                                                                                <tr key={detail.id || i}>
                                                                                    ...
                                                                                    <td className="py-3 px-4 text-gray-400 text-left text-custom-sm">{detail.invoice_no}</td>
                                                                                    ...
                                                                                    <td className="py-3 px-4 text-gray-400 text-center text-custom-sm">{detail.ex_rate_diff}</td>
                                                                                </tr>
                                                                            ))
                                                                        ) : (
                                                                            <tr>
                ...
                <div className="border-t flex items-center justify-between" style={{ borderColor: "#404040" }}>
                    <div className="p-2 flex items-center space-x-1">
                        ...
                        />
                        <ExportReportSelector formats={["odt", "ods", "xlsx"]} baseName="ReceiveVoucher" ids={selectedRV.length > 0 ? selectedRV : rvoucherList.map((p) => p.id)} language={lang} />
                    </div>
