...
import { fetchInvoiceStatus, fetchBank, fetchTaxGroup, fetchPaymentType, convertToSingleOption, OptionType } from "@/utils/fetchDropdownData";
...
interface PaymentVoucherDetailsProps {
    ...
    onSave: () => void;
    tabId: string;
    onChangePaymentVoucherId: (newId: number) => void;
...
const PaymentVoucherDetails: React.FC<PaymentVoucherDetailsProps> = ({ paymentVoucherId, tabId, saveType, onBack, onSave, onChangePaymentVoucherId, onChangeSaveType }) => {
    const { translations, lang } = useLanguage();
    const [masterList, setTransDetails] = useState<ApiPaymentVoucher | null>(null);
    const [selectedForCredits, setSelectedCredits] = useState<ApiPaymentVoucher[]>([]);
    ...
    const [selectedDetails, selectedPaymentVoucher] = useState<number[]>([]);
    const [totalPagesSupplier, setTotalPages_Supplier] = useState(1);
    const [supplierInvoicePopup, setShowSupplierInvoice] = useState(false);
    ...
    useEffect(() => {
        const payment_type_id = formData.payment_type_id?.value ?? 1;
        setDefaultPaymentType(Number(payment_type_id));
    ...
    const fetchChartsOfAccounts = async (page = currentPageAccounts, perPage = itemsPerPageAccounts, search = "") => {
        try {
            ...
            const paginatedData = await paymentVoucherService.getPVChartsOfAccount(page, perPage, search, "");
    ...
    const handleSave = async () => {
        ...
        if (!formData.payment_type_id) {
            showErrorToast(translations["Payment Type is required"] || "Payment Type is required");
            setLoadingSave(false); // Enable button again
            return;
        }
        if (Number(formData.payment_type_id.value) === 1 && formData.supplier_id === 0) {
            showErrorToast(translations["Pay To is required"] || "Pay To is required");
            setLoadingSave(false); // Enable button again
            return;
        }
        if (Number(formData.payment_type_id.value) === 2 && formData.pay_to_en === "" && lang === "en") {
            showErrorToast(translations["Pay To is required"] || "Pay To is required");
            setLoadingSave(false); // Enable button again
            return;
        }
        if (Number(formData.payment_type_id.value) === 2 && formData.pay_to_cn === "" && lang === "cn") {
            showErrorToast(translations["Pay To is required"] || "Pay To is required");
            setLoadingSave(false); // Enable button again
            return;
        }
        if (Number(formData.payment_type_id.value) === 3 && formData.customer_id === 0) {
            showErrorToast(translations["Pay To is required"] || "Pay To is required");
            setLoadingSave(false); // Enable button again
            return;
        }
        if (Number(formData.payment_type_id.value) === 5 && formData.supplier_id === 0) {
            showErrorToast(translations["Pay To is required"] || "Pay To is required");
            setLoadingSave(false); // Enable button again
            return;
        }
        ...
        try {
            ...
            if ((saveType === "new" || saveType === "copy") && newId) {
                onChangePaymentVoucherId(newId);
    ...
    const handleSelectCustomer = (id: number, checked: boolean) => {
        if (checked) {
            selectedPaymentVoucher((prev) => [...prev, id]);
        } else {
            selectedPaymentVoucher((prev) => prev.filter((pid) => pid !== id));
        }
    };
        ...
        switch (column) {
            case "account_code":
                try {
                    ...
                    console.log(ageType);
                    const res = await paymentVoucherService.getPVChartsOfAccount(currentPageAccounts, itemsPerPageAccounts, "", value);
                ...
                break;
            case "customer_code":
                const res = await paymentVoucherService.getCustomerInfoByCode(value, "enter");
                ...
                const pv_status_id0 = convertToSingleOption(7, invoiceStatusOptions);
                const tax_group0 = convertToSingleOption(res.list.tax_group, taxGroupOptions);
                const currency0 = String(res.list.currency);
                const exRate0 = await productService.getCurrentDateExRate(currency0, baseCurrency());
                if (parseFloat(exRate0.toString()) === 0) {
                    showErrorToast(translations["Exchange Rate is Required"]);
                    return;
                }
                setFormData((prev) => ({
                    ...prev,
                    customer_id: Number(res.list.customer_id),
                    customer_code: String(res.list.customer_code),
                    supplier_id: 0,
                    supplier_code: "",
                    currency: String(res.list.currency),
                    address_en: String(res.list.billing_address_en),
                    address_cn: String(res.list.billing_address_cn),
                    pv_status_id: pv_status_id0,
                    ex_rate: exRate0,
                    tax_group: tax_group0,
                }));
                setShowCustomers(false);
                break;
            case "supplier_code":
                const suppList = await paymentVoucherService.getPOSupplierInfo(value);
                if (suppList.token === "SupplierNotExists") {
                    showErrorToast(translations[suppList.message]);
                    return;
                }
                if (suppList.token === "MultipleSupplier") {
                    setShowSuppliers(true);
                    return;
                }
                const currency = suppList.list.currency;
                const pv_status_id = convertToSingleOption(7, invoiceStatusOptions);
                const exRate = await productService.getCurrentDateExRate(currency, baseCurrency());
                if (parseFloat(exRate.toString()) === 0) {
                    showErrorToast(translations["Exchange Rate is Required"]);
                    return;
                }
                const tax_group = convertToSingleOption(suppList.list.tax_group, taxGroupOptions);
                setFormData((prev) => ({
                    ...prev,
                    supplier_id: Number(suppList.list.supplier_id),
                    supplier_code: String(suppList.list.supplier_code),
                    currency: String(suppList.list.currency),
                    address_en: String(suppList.list.supplier_address_en),
                    address_cn: String(suppList.list.supplier_address_cn),
                    pv_status_id: pv_status_id,
                    ex_rate: exRate,
                    tax_group: tax_group,
                }));
                if (defaultPaymentType === 1) {
                    const listArr = await paymentVoucherService.doGetSupplierInvoice(Number(suppList.list.supplier_id));
                    const len = listArr.length;
                    if (len > 0) {
                        setShowSupplierInvoice(true);
                        setSelectedCredits(listArr);
                    }
                }
                break;
        }
    ...
    const handleRowClick_Customer = async (e: React.MouseEvent, tableId: number) => {
        ...
        const res = await paymentVoucherService.getCustomerInfoByCode(String(customer_code), "click");
        if (res.token === "CustomerNotExists") {
        ...
        const currency = String(res.list.currency);
        const exRate = await productService.getCurrentDateExRate(currency, baseCurrency());
        if (parseFloat(exRate.toString()) === 0) {
            showErrorToast(translations["Exchange Rate is Required"]);
            return;
    ...
    const handleRowClick_Supplier = async (e: React.MouseEvent, tableId: number) => {
        ...
        const pv_status_id = convertToSingleOption(7, invoiceStatusOptions);
        const tax_group = convertToSingleOption(specificItem?.tax, taxGroupOptions);
        const currency = String(specificItem?.currency);
        const payment_type_id = Number(formData.payment_type_id?.value);
        const exRate = await productService.getCurrentDateExRate(currency, baseCurrency());
        if (parseFloat(exRate.toString()) === 0) {
            showErrorToast(translations["Exchange Rate is Required"]);
            return;
        ...
        setFormData((prev) => ({
            ...
            customer_id: 0,
            customer_code: "",
            supplier_id: Number(specificItem?.id),
            supplier_code: String(supplier_code),
            currency: String(specificItem?.currency),
            address_en: String(specificItem?.supplier_address_en),
            address_cn: String(specificItem?.supplier_address_cn),
            pv_status_id: pv_status_id,
            ex_rate: exRate,
            tax_group: tax_group,
        }));
        if (payment_type_id === 1) {
            const listArr = await paymentVoucherService.doGetSupplierInvoice(Number(specificItem?.id));
            const len = listArr.length;
            if (len > 0) {
                setShowSupplierInvoice(true);
                setSelectedCredits(listArr);
    ...
    const handleRowClick_SuppInvoice = async (e: React.MouseEvent, apNumber: string, account_code: string, ap_id: number) => {
        // Prevent navigation if clicking on checkboxes or toggle switches
        const target = e.target as HTMLElement;
        if (target.closest("input") || target.closest("label") || target.closest("[data-copy-button]")) {
            return;
        }
        const res = await paymentVoucherService.doGetAPInvoiceDetail(apNumber, account_code);
        setDetailList([]);
        res.forEach((element: any) => {
            ...
            const newData: DetailsRow = {
                ...
                po_detail_id: element.po_detail_id,
                ap_detail_id: ap_id,
                qty: element.qty,
                account_no: "",
                ap_invoice_no: element.ap_number,
                ref_data: element.ap_number,
            ...
            setDetailList((prev) => [...prev, newData]);
            handleDetailChange(newId, element.total, "amount");
        });
        setShowSupplierInvoice(false);
    };
                                ...
                                <div className="col-span-12 md:col-span-4">
                                    ...
                                    <div className="flex items-center gap-4 mb-2">
                                        ...
                                        <div className="flex flex-1">
                                            {(defaultPaymentType === 1 || defaultPaymentType === 5) && (
                                                <input
                                                    type="text"
                                            ...
                                            {defaultPaymentType !== 2 && (
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        if (defaultPaymentType === 1 || defaultPaymentType === 5) {
                                                            setShowSuppliers(true);
                                                        }
                                                    }}
                                    ...
                                    <table className="w-full border border-gray-600 border-collapse text-sm text-left">
                                        <thead className="sticky top-0 z-[1] py-1 px-4" style={{ backgroundColor: "#1f2132" }}>
                                            <tr className="border-b" style={{ borderColor: "#2d2d30" }}>
                                                ...
                                                <th className={`w-[8%] text-left py-1 px-2 text-gray-400 text-sm border border-[#40404042]`}>{translations["Account Code"]}</th>
                                                <th className={`w-[18%] text-left py-1 px-2 text-gray-400 text-sm border border-[#40404042]`}>{translations["Description"]}</th>
                                                <th className={`w-[21%] text-left py-1 px-2 text-gray-400 text-sm border border-[#40404042]`}>{translations["Product Code"]}</th>
                                                <th className={`w-[7%] text-center py-1 px-2 text-gray-400 text-sm border border-[#40404042]`}>{translations["Currency"]}</th>
                                                <th className={`w-[9%] text-center py-1 px-2 text-gray-400 text-sm border border-[#40404042]`}>{translations["Ex Rate"]}</th>
                                                <th className={`w-[9%] text-center py-1 px-2 text-gray-400 text-sm border border-[#40404042]`}>{translations["Amount"]}</th>
                                                <th className={`w-[9%] text-center py-1 px-2 text-gray-400 text-sm border border-[#40404042]`}>{translations["Default Currency"]}</th>
                                                <th className={`w-[9%] text-left py-1 px-2 text-gray-400 text-sm border border-[#40404042]`}>{translations["Reference"]}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {detailList.length === 0 ? (
                                                        ...
                                                        item.is_deleted !== 1 && (
                                                            ...
                                                            >
                                                                ...
                                                                <td className="py-0 px-0 text-gray-400 text-left text-custom-sm border border-[#40404042]">
                                                                    <input
                                                                        type="text"
                                                                        value={item.product_code + " " + (lang === "en" ? item.product_title_en : item.product_title_cn)}
                                                                        readOnly
    ...
    const renderSupplierInvoices = () => {
        if (!supplierInvoicePopup) return null;
        return (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-[#19191c] rounded-xl border border-[#ffffff1a] w-[30vw] h-auto flex flex-col">
                    {/* Header */}
                    <div className="flex items-center justify-between p-4 border-b border-[#ffffff1a]">
                        <h2 className="text-xl font-bold text-white">{translations["Search Supplier Invoice"]}</h2>
                    </div>
                    {/* Content */}
                    <div className="p-4">
                        <div className="col-span-12 md:col-span-12">
                            <table className="w-full border border-gray-600 border-collapse text-sm text-left">
                                <thead className="sticky top-0 z-[1] py-1 px-4" style={{ backgroundColor: "#1f2132" }}>
                                    <tr className="border-b" style={{ borderColor: "#2d2d30" }}>
                                        <th className="text-left py-2 px-2 text-gray-400 text-sm w-12 border border-[#40404042]">{translations["A/P Invoice No."]}</th>
                                        <th className="text-right py-2 px-2 text-gray-400 text-sm w-12 border border-[#40404042]">{translations["Balance To Pay"]}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {selectedForCredits.length === 0 ? (
                                        ...
                                        selectedForCredits.map((item) => (
                                            <tr
                                                ...
                                                onClick={(e) => handleRowClick_SuppInvoice(e, item.ap_invoice_no, item.account_code, item.ap_detail_id)}
                                                className={`border-b hover:bg-gray-700 hover:bg-opacity-30 transition-colors cursor-pointer`}
                                                style={{ borderColor: "#40404042" }}
                                            >
                                                <td className="py-2 px-2 text-gray-400 text-left text-custom-sm border border-[#40404042]">{item.ap_invoice_no}</td>
                                                <td className="py-2 px-2 text-gray-400 text-right text-custom-sm border border-[#40404042]">{formatMoney(item.balance)}</td>
                                            </tr>
                                        ))
                                    )}
                                </tbody>
                    ...
                    <div className="p-2 border-t border-[#ffffff1a] flex justify-end space-x-2">
                        <button
                            onClick={() => setShowSupplierInvoice(false)}
