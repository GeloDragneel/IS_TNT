...
import { baseCurrency, formatMoney, formatDate } from "@/utils/globalFunction";
...
const SupplierDashboard: React.FC = () => {
    ...
    const [selectedMetric, setSelectedMetric] = useState<"total_sales" | "total_deposit" | "total_count">("total_sales");
    useEffect(() => {
        const fetchDashboard = async () => {
            const data = await dashboardService.getDashboardPO(Number(selectedMonthYear.month + 1), Number(selectedMonthYear.year));
            ...
            const mappedTopSupplier: DashboardFields[] = topSupplier.map((item: any) => ({
            ...
            const mappedToSalesPerson: DashboardFields[] = topSalesPerson.map((item: any) => ({
                name: lang === "en" ? item.postatus_en : item.postatus_cn,
                value: item.count,
                color: item.color || "bg-gray-500", // fallback color
            }));
    ...
    const metricSettings = {
        total_sales: { label: translations["Total"], color: "#4ade80", dataKey: "total_sales" },
        total_deposit: { label: translations["Deposit"], color: "#3b82f6", dataKey: "total_deposit" },
        total_count: { label: translations["Count"], color: "#eab308", dataKey: "total_count" },
    };
    ...
    const pieDataCustomer = topSuppliers.map((cat) => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
                    ...
                    <div className="md:col-span-6 p-6 rounded-2xl border shadow-lg" style={{ backgroundColor: "#19191c", borderColor: "#404040", height: "auto", paddingBottom: 20 }}>
                        <div className="flex items-center justify-between mb-1">
                            <h2 className="text-xl font-bold text-white">
                                {translations["Purchase Summary"] || "Purchase Summary"} <span>{selectedMonthYear.year}</span>
                            </h2>
                            ...
                            <div className="flex items-center gap-1 relative">
                                ...
                                <div className="flex gap-1">
                                    {Object.keys(metricSettings).map((key) => (
                                        <button
                                            key={key}
                                            onClick={() => setSelectedMetric(key as "total_sales" | "total_deposit" | "total_count")}
                    ...
                    <div className="md:col-span-4 p-0 rounded-xl shadow-sm">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-2">
                            {[
                                {
                                    title: translations["Suppliers"] || "Suppliers",
                                ...
                                {
                                    title: translations["PO Count"] || "PO Count",
                                    value: summary?.POCount || 0,
                                    icon: PackageCheck,
                                    bg: "bg-green-900/50",
                                    iconColor: "text-green-400",
                                },
                                {
                                    title: translations["Invoice Count"] || "Invoice Count",
                                ...
                                {
                                    title: translations["PO Amount"] || "PO Amount",
                                    value: summary?.POAmount || 0,
                                    icon: Receipt,
                                    bg: "bg-blue-900/50",
                                    iconColor: "text-blue-400",
                                },
                                {
                                    title: translations["Invoice Amount (Paid)"] || "Invoice Amount (Paid)",
                                    value: summary?.InvoiceAmountPaid || 0,
                                    icon: Wallet,
                                    bg: "bg-cyan-900/50",
                                ...
                                {
                                    title: translations["PO Deposit (Paid)"] || "PO Deposit (Paid)",
                                    value: summary?.PODepositPaid || 0,
                                    icon: ArrowUpCircle,
                                    bg: "bg-yellow-900/50",
                                    iconColor: "text-yellow-400",
                                },
                                {
                                    title: translations["Invoice Amount (Unpaid)"] || "Invoice Amount (Unpaid)",
                                    value: summary?.InvoiceAmountUnpaid || 0,
                                    icon: ArrowDownCircle,
                                    bg: "bg-teal-900/50",
                                    iconColor: "text-teal-400",
                                },
                                {
                                    title: translations["PO Deposit (Unpaid)"] || "PO Deposit (Unpaid)",
                                    value: summary?.PODepositNotPaid || 0,
                                    icon: PackageCheck,
                                    bg: "bg-purple-900/50",
                    ...
                    >
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-bold text-white">{translations?.["PO Performance By Supplier"] || "PO Performance By Supplier"}</h2>
                            <div className="flex items-center gap-1 text-gray-400 text-xs px-1 py-1 border-gray-700">
                                <span>
                                    {months[selectedMonthYear.month]} {selectedMonthYear.year}
                                </span>
                    ...
                    >
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-bold text-white">{translations?.["Performance By PO Status"] || "Performance By PO Status"}</h2>
                            <div className="flex items-center gap-1 text-gray-400 text-xs px-1 py-1 border-gray-700">
                                <span>
                                    {months[selectedMonthYear.month]} {selectedMonthYear.year}
                                </span>
                    ...
                    >
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-bold text-white">{translations?.["Latest PO Received"] || "Latest PO Received"}</h2>
                            <div className="flex items-center gap-1 text-gray-400 text-xs px-1 py-1 border-gray-700">
                                <span>
                                    {months[selectedMonthYear.month]} {selectedMonthYear.year}
                                </span>
                        ...
                        <div className="flex-grow overflow-y-auto space-y-6 pr-2">
                            {topProducts.map(({ rank, name, date, salesChange, image }, idx) => (
                                <div key={idx} className="flex items-center justify-between border-b border-gray-700 pb-2 last:border-b-0">
                                    ...
                                    <div className="flex-1 ml-3">
                                        ...
                                        <div className="text-xs text-gray-400">
                                            <span className="text-blue-400 cursor-pointer">{formatDate(date, lang)}</span>
                                        </div>
                                    </div>
                                    <div className="flex flex-col items-end ml-4 min-w-[70px]">
                                        <div className="flex items-center text-xs font-medium">
                                            <span>{translations["Received Qty"] || "Received Qty"}</span>
