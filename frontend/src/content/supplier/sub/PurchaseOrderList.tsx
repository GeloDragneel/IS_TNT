...
import { fetchSuppliers, fetchPOStatus, fetchBank, OptionType, convertToSingleOption } from "@/utils/fetchDropdownData";
...
interface PurchaseOrderListProps {
    tabId: string;
    onPurchaseOrderSelect: (supplierId: number, saveType?: string) => void;
...
// localStorage.clear();
const PurchaseOrderList: React.FC<PurchaseOrderListProps> = ({ tabId, onPurchaseOrderSelect, onChangeView, selectedPurchaseOrder, onSelectedPurchaseOrderChange }) => {
    const { translations, lang } = useLanguage();
    const [purchaseOrderList, setPurchaseOrder] = useState<ApiPurchaseOrder[]>([]);
    ...
    const exportRef2 = useRef<{ triggerExport: () => void }>(null);
    ...
    useEffect(() => {
        ...
        if (cachedExpandedKey) {
            ...
            parseExpanded.forEach((tableId: number) => {
                ...
                const data = purchaseOrderList.find((po) => po.id === tableId);
                ...
                const detailsCopy: DetailsExpanded[] = data.details_copy;

                const mergedDetails = [...details, ...detailsCopy]; // Simple merge
                setPODetailsMap((prev) => ({ ...prev, [tableId]: mergedDetails }));
            });
    ...
    useEffect(() => {
        ...
        channel.listen(".po-event", () => {
            ...
            fetchPurchaseOrder(currentPage, itemsPerPage, defaultFilters);
        });
        return () => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (isCacheValid) {
                setPurchaseOrder(cachedSuppliers);
    ...
    useEffect(() => {
        if (selectedPurchaseOrder.length === 0) {
            ...
            const selectedPurchaseOrders = purchaseOrderList.filter((product) => selectedPurchaseOrder.includes(product.id));
            let isDeposit = 0;
            let totalDeposit = 0;
            selectedPurchaseOrders.forEach((element) => {
                const deposit = element.deposit || 0;
                const depositJV = element.deposit_jv.length;
                const depositPV = element.deposit_pv.length;
                if (depositPV == 0 && deposit == 0) {
                    isDeposit = 1;
                } else if (depositPV > 0 && deposit > 0) {
                    isDeposit = 1;
                } else if (depositJV == 0 && deposit == 0) {
                    isDeposit = 1;
                } else if (depositJV > 0 && deposit > 0) {
                    isDeposit = 1;
                } else {
    ...
    const fetchPurchaseOrder = async (page = currentPage, perPage = itemsPerPage, filters: FilterParams = { search: "" }) => {
        try {
            if (loading) {
                setLoading(true);
            }
            localStorage.setItem(`${tabId}-loading-purchase-order`, JSON.stringify(true));
            const paginatedData = await purchaseOrderService.getAllPurchaseOrer(page, perPage, filters);
            setPurchaseOrder(paginatedData.data);
            setTotalPages(paginatedData.last_page);
            setPreorderFooter2(paginatedData.footer2);
            console.log(paginatedData.footer2);
    ...
    // Filter purchaseOrderList based on search term
    // Handle select all
    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            onSelectedPurchaseOrderChange(purchaseOrderList.map((p) => p.id).filter(Boolean) as number[]);
        } else {
    ...
    const statusMap: Record<string, { color: string }> = {
        ...
        4: { color: "bg-red-600 bg-opacity-20 text-red-400 uppercase" },
    };
    ...
    const handleDateRange = () => {
        ...
        fetchPurchaseOrder(1, itemsPerPage, filters);
    };
    const handleReset = () => {
        setFormData({
            ...formData,
    ...
    const handleVoidSelected = async () => {
        ...
        if (selectedPurchaseOrder.length > 1) {
        ...
        let poId = 0;
        selectedPurchaseOrders.forEach((element) => {
            const data = purchaseOrderList.find((po) => po.id === element.id);
            // Save to state
            if (!data?.details) {
            ...
            if (!data?.pv_details) {
                console.error("No PV details found for this PO");
                return;
            }
            const details: DetailsExpanded[] = data.details;
            const pv_details: DetailsExpanded[] = data.pv_details;
            details.forEach((list) => {
                totalReceiveQty += list.receive_qty;
            });
            pv_details.forEach((list) => {
                totalPaidPV += list.amount;
            });
        ...
        if (!confirmed) return;
        try {
            const res = await purchaseOrderService.voidPurchaseOrder(poNumber);
            if (res.token === "Error") {
                showErrorToast(res.message);
                return;
            }
            onSelectedPurchaseOrderChange([]);
            fetchPurchaseOrder(currentPage, itemsPerPage, searchTerm);
            showSuccessToast(translations["Record(s) Voided"]);
            localStorage.removeItem(`${poId}-cached-po-info`);
        } catch (err) {
    ...
    const handleSaveCancelPO = async (type: string) => {
        ...
        const selectedPurchaseOrders = purchaseOrderList.filter((supplier) => selectedPurchaseOrder.includes(supplier.id));
        if (selectedPurchaseOrders.length === 0 || !selectedPurchaseOrders[0].po_number) {
            showErrorToast(translations["No valid PO selected"]);
            return;
        }
        const po_number = selectedPurchaseOrders[0].po_number as string;
        const poId = selectedPurchaseOrders[0].id;
        // Set loading states
        if (type === "CTOSUPP") setLoadCancelOrder_CTOSUPP(true);
        if (type === "MSCLSS") setLoadCancelOrder_MSCLSS(true);
        if (type === "RPTOS") setLoadCancelOrder_RPTOS(true);

        try {
            const result = await purchaseOrderService.cancelPurchaseOrder(po_number, type);
            ...
            // Clear all loading states regardless of success/failure
    ...
    const handleRowClick = (e: React.MouseEvent, supplierId: number) => {
        ...
        onPurchaseOrderSelect(supplierId, supplierId == 0 ? "new" : "edit");
    };
    ...
    const handleToggleRow = async (tableId: number) => {
        ...
        if (isExpanded) {
            ...
            try {
                ...
                const details: DetailsExpanded[] = data.details;
                const detailsCopy: DetailsExpanded[] = data.details_copy;
                const mergedDetails = [...details, ...detailsCopy]; // Simple way
                setPODetailsMap((prev) => ({ ...prev, [tableId]: mergedDetails }));
                localStorage.setItem(cachedKey, JSON.stringify(mergedDetails));
            } catch (error) {
                ...
                <div className="bg-[#19191c] rounded-xl border border-[#ffffff1a] w-[60vw] max-h-[80vh] h-auto flex flex-col">
                    ...
                    <div className="grid grid-cols-12 gap-4 p-4">
                        ...
                        <div className="col-span-12">
                            <label className="text-gray-400 text-sm">{translations["Status"]}</label>
                            <Select
                                classNamePrefix="react-select"
                                isMulti
                                value={formData.poStatus}
                                onChange={(selected) => {
                                    setFormData({ ...formData, poStatus: selected as OptionType[] });
                                }}
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-[#19191c] rounded-xl border border-[#ffffff1a] w-[25vw] h-auto flex flex-col">
                    ...
                    <div className="p-2 border-t border-[#ffffff1a] flex justify-end space-x-2">
                        ...
                        <button
                            onClick={() => {
                                exportRef2.current?.triggerExport();
                            }}
                            className="px-2 py-2 bg-cyan-600 hover:bg-cyan-500 rounded text-white transition focus:outline-none focus:ring-2 focus:ring-cyan-500"
                        >
                            {translations["Print"]}
                ...
                <div className="p-4 border-b flex-shrink-0" style={{ borderColor: "#404040" }}>
                    <div className="flex justify-between items-center">
                        <div className="flex items-center space-x-4">
                            ...
                            <button
                                onClick={() => onChangeView("depositPaid")}
                        ...
                        <div className="flex items-center space-x-4">
                            <div className="flex items-center space-x-1">
                                <button
                                    ...
                                    disabled={disabledCreatePV || selectedPurchaseOrder.length === 0}
                                ...
                                <button
                                    onClick={() => setShowPrintBySupplier(true)}
                                    className="px-2 py-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded-lg transition-colors flex items-center space-x-1 text-sm"
                                >
                                    <span>{translations["Print By Supplier"]}</span>
                                </button>
                                <button
                                    disabled={selectedPurchaseOrder.length === 0}
                                ...
                                <tr className="border-b" style={{ borderColor: "#2d2d30" }}>
                                    ...
                                    <th className="w-[3%] text-left py-1 px-2 text-gray-400 text-sm w-12">
                                        <div className="flex items-center h-full">
                                            <CustomCheckbox
                                                checked={selectedPurchaseOrder.length === purchaseOrderList.length && purchaseOrderList.length > 0}
                                                onChange={(checked) => handleSelectAll(checked)}
                                            />
                                        </div>
                                    </th>
                                    <th className="w-[10%] text-left py-2 px-2 text-gray-400 text-sm">{translations["PO Number"]}</th>
                                    <th className="w-[10%] text-left py-2 px-2 text-gray-400 text-sm">{translations["PO Date"]}</th>
                                    <th className="w-[10%] text-left py-2 px-2 text-gray-400 text-sm">{translations["Supplier Code"]}</th>
                                    <th className="w-[20%] text-left py-2 px-2 text-gray-400 text-sm">{translations["Supplier Name"]}</th>
                                    <th className="w-[8%] text-center py-2 px-2 text-gray-400 text-sm">{translations["PO Amount"]}</th>
                                    <th className="w-[8%] text-center py-2 px-2 text-gray-400 text-sm">{translations["Deposit"]}</th>
                                    <th className="w-[7%] text-center py-2 px-2 text-gray-400 text-sm">{translations["Ex Rate"]}</th>
                                    <th className="w-[10%] text-left py-2 px-2 text-gray-400 text-sm">{translations["Deposit PV"]}</th>
                                    <th className="w-[9%] text-left py-2 px-2 text-gray-400 text-sm">{translations["Status"]}</th>
                                </tr>
                            </thead>
                            {loading ? (
                                ...
                                <tbody>
                                    {purchaseOrderList.map((supplier, index) => (
                                        <React.Fragment key={supplier.id || index}>
                                            ...
                                            >
                                                ...
                                                <td className="py-2 px-2 text-gray-400 text-left text-custom-sm">{formatDate(supplier.po_date, lang)}</td>
                                                <td className="py-2 px-2 text-gray-400 text-left text-custom-sm">
                                                    <div className="group flex items-center">
                                                ...
                                                <td className="py-2 px-2 text-gray-400 text-center text-custom-sm">
                                                    {supplier.currency}
                                                    <br></br>
                                                    {formatMoney(supplier.po_amount)}
                                                </td>
                                                <td className="py-2 px-2 text-gray-400 text-center text-custom-sm">
                                                    {supplier.currency}
                                                    <br></br>
                                                ...
                                                <td className="py-2 px-2 text-gray-400 text-left text-custom-sm">
                                                    <div className="group flex items-center">
                                                        <p className="text-gray-400 text-custom-sm">{highlightMatch(supplier.deposit_pv || translations["N.A."], searchTerm)}</p>
                                                        <CopyToClipboard text={supplier.deposit_pv ?? ""} />
                                                    </div>
                                                </td>
                                                <td className="py-2 px-2 text-gray-400 text-left text-custom-sm">
                                                    <span className={`px-2 py-1 rounded-full text-xs text-custom-sm ${getStatusColor(supplier.postatus_id)}`}>
                                                        {lang === "en" ? supplier.postatus_en : supplier.postatus_cn}
                                                    </span>
                                                </td>
                                            </tr>
                                                    ...
                                                    <td colSpan={12} className="py-2 px-2">
                                                        <table className="w-full" style={{ backgroundColor: "#060818" }}>
                                                            <thead className="sticky top-0 z-[1] py-1 px-4" style={{ backgroundColor: "#1f2132" }}>
                                                                <tr className="border-b" style={{ borderColor: "#2d2d30" }}>
                                                                    <th className="w-auto text-left py-2 px-2 text-gray-400 text-sm w-auto border border-[#40404042]">
                                                                        {translations["Product Code"]}
                                                                    </th>
                                                                    <th className="w-auto text-left py-2 px-2 text-gray-400 text-sm w-auto border border-[#40404042]">
                                                                        {translations["Product Name"]}
                                                                    </th>
                                                                    <th className="w-auto text-center py-2 px-2 text-gray-400 text-sm w-auto border border-[#40404042]">{translations["Qty"]}</th>
                                                                    <th className="w-auto text-center py-2 px-2 text-gray-400 text-sm w-auto border border-[#40404042]">
                                                                        {translations["Unit Price"]}
                                                                    </th>
                                                                    <th className="w-auto text-center py-2 px-2 text-gray-400 text-sm w-auto border border-[#40404042]">{translations["Total"]}</th>
                                                                    <th className="w-auto text-center py-2 px-2 text-gray-400 text-sm w-auto border border-[#40404042]">{translations["Deposit"]}</th>
                                                                    <th className="w-auto text-left py-2 px-2 text-gray-400 text-sm w-auto border border-[#40404042]">{translations["Deposit PV"]}</th>
                                                                    <th className="w-auto text-left py-2 px-2 text-gray-400 text-sm w-auto border border-[#40404042]">{translations["Invoice PV"]}</th>
                                                                    <th className="w-auto text-center py-2 px-2 text-gray-400 text-sm w-auto border border-[#40404042]">
                                                                        {translations["Receive Date"]}
                                                                    </th>
                                                                    <th className="w-auto text-center py-2 px-2 text-gray-400 text-sm w-auto border border-[#40404042]">
                                                                        {translations["Receive Qty"]}
                                                                    </th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                    ...
                                                                    orderDetailsMap[supplier.id]!.map((detail, i) => (
                                                                        <tr key={detail.id || i}>
                                                                            <td className="py-2 px-2 text-gray-400 text-left text-custom-sm border border-[#40404042]">
                                                                                <div className="group flex items-center">
                                                                                    <p className="text-gray-400 text-custom-sm">{highlightMatch(detail.product.product_code, searchTerm)}</p>
                                                                                    <CopyToClipboard text={detail.product.product_code} />
                                                                                </div>
                                                                            </td>
                                                                            <td className="py-2 px-2 text-gray-400 text-left text-custom-sm border border-[#40404042]">
                                                                                {highlightMatch(lang === "en" ? detail.product.product_title_en : detail.product.product_title_cn, searchTerm)}
                                                                            </td>
                                                                            ...
                                                                            <td className="py-2 px-2 text-gray-400 text-left text-custom-sm border border-[#40404042]">
                                                                                <p className="text-gray-400 text-custom-sm">{highlightMatch(detail.deposit_pv || translations["N.A."], searchTerm)}</p>
                                                                            </td>
                                                                            <td className="py-2 px-2 text-gray-400 text-left text-custom-sm border border-[#40404042]">
                                                                                <p className="text-gray-400 text-custom-sm">{highlightMatch(detail.invoice_pv || translations["N.A."], searchTerm)}</p>
                                                                            </td>
                                                                            <td className="py-2 px-2 text-gray-400 text-center text-custom-sm border border-[#40404042]">
                                                                                {formatDate(detail.receive_date, lang)}
                                                                            </td>
                                                                            <td className="py-2 px-2 text-gray-400 text-center text-custom-sm border border-[#40404042]">{detail.receive_qty}</td>
                                                                        </tr>
                                                                    ))
                                                                ) : (
                                                                    <tr>
                ...
                <div className="border-t flex items-center justify-between" style={{ borderColor: "#404040" }}>
                    <div className="p-4 flex items-center space-x-1">
                        ...
                        <ExportReportSelector
                            ...
                            ids={selectedPurchaseOrder.length > 0 ? selectedPurchaseOrder : purchaseOrderList.map((p) => p.id)}
                            language={lang}
                        />
                        <div className="hidden">
                            <ExportReportSelector ref={exportRef} formats={["odt"]} baseName="DownloadSelectedSinglePO" ids={selectedPurchaseOrder} language={lang} />
                            <ExportReportSelector ref={exportRef2} formats={["odt"]} baseName="PO_Report_By_Supplier" ids={[Number(formData.supplierCodeOnPrint?.value)]} language={lang} />
                        </div>
                    </div>
