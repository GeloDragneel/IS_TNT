...
import { salesOrderService, ApiSalesOrder, DetailsRow } from "@/services/salesOrderService";
...
import { fetchPaymentTerms, fetchShippingTerms, fetchShippingStat, fetchInvoiceType, fetchTaxGroup, convertToSingleOption, OptionType } from "@/utils/fetchDropdownData";
...
interface SalesOrderDetailsProps {
    ...
    onSave: () => void;
    tabId: string;
    onChangeSalesOrderId: (newId: number) => void;
    onChangeSaveType: (type: string) => void;
}
const defaultDetails: ApiSalesOrder = {
    id: 0,
    invoice_no: "",
    cnt_products: 0,
    customer_id: 0,
...
const SalesOrderDetails: React.FC<SalesOrderDetailsProps> = ({ salesOrderId, tabId, saveType, onBack, onSave, onChangeSalesOrderId, onChangeSaveType }) => {
    const { translations, lang } = useLanguage();
    const [masterList, setTransDetails] = useState<ApiSalesOrder | null>(null);
    const [SOOrderIds, setSOOrderIds] = useState<ApiSalesOrder[]>([]);
    const [selectedItems, setSelectedItems] = useState<ApiSalesOrder[]>([]);
    const [selectedForCredits, setSelectedCredits] = useState<ApiSalesOrder[]>([]);
    ...
    const [loadingCreateInv, setLoadingCreateInv] = useState(false);
    ...
    useEffect(() => {
        const calculateTotalsAndFetch = async () => {
            ...
            totalCRAmount = formData.cr_amount;
            totalAdvAmount = formData.adv_amount;
            totalExcessAmount = formData.excess_amount;
            total = subTotal + taxAmount;
            totalCreditUsed = totalCRAmount + totalAdvAmount + totalExcessAmount;
            totalDeduction = totalDeposit + totalCreditUsed + totalVoucherAmount;
            totalToPay = total - totalDeduction;
    ...
    const fetchSOInfo = async () => {
        try {
            const idToUse = salesOrderId;
            if (!idToUse) {
                setLoading(false);
                return;
            }
            ...
            const purchaseOrderInfoKey = `${idToUse}-cached-so-info`;
            ...
            if (cachedSuppliersRaw) {
                ...
                foundSupplier = await salesOrderService.getSOInfo(idToUse); // ✅ Fetch from API
            }
            if (foundSupplier) {
                ...
                setFormData((prev) => ({
                    ...
                    so_date: parseDate(foundSupplier.so_date),
    ...
    const fetchProducts = async (page = currentPageProduct, perPage = itemsPerPageProduct, search = "", sortId = selectedChkProducts, prodType = activeDropdownProd) => {
        try {
            ...
            const paginatedData = await salesOrderService.getAllSOProducts(page, perPage, search, sortId, prodType);
    ...
    const updateCachedRaw = async (cachedType: string, masterId: number) => {
        if (cachedType === "info") {
            const newData = await salesOrderService.getSOInfo(masterId);
            if (newData) {
                localStorage.setItem(`${masterId}-cached-so-info`, JSON.stringify(newData));
            }
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            const result = await salesOrderService.updateSalesOrder(salesOrderId, data);
    ...
    const handleAddNew = async () => {
        const confirmed = await showConfirm2(
            translations["Issue New SO for this customer"],
            translations["Issue New SO for this customer"],
            translations["Yes"],
            translations["No"],
            translations["Cancel"]
    ...
    const handleCreateInvoice = async () => {
        const confirmed = await showConfirm(translations["Create Invoice"], translations["Are you sure you want to continue"] + "?", translations["Yes"], translations["Cancel"]);
        if (!confirmed) return;
        setLoadingCreateInv(true);
        try {
            const result = await salesOrderService.convertToInvoice(formData.so_number);
            if (result.token === "Error") {
                setLoadingCreateInv(false);
                showErrorToast(result.message);
                return;
            }
            await updateCachedRaw("info", salesOrderId);
            fetchSOInfo();
            setLoadingCreateInv(false);
            showSuccessToast(result.invoice_no + " " + translations[result.message]);
        } catch (error) {
            setLoadingCreateInv(false);
        } finally {
            setLoadingCreateInv(false);
            ...
            case "new":
                ...
                for (const element of selectedChkProducts) {
                    try {
                        ...
                        const onProduct = await salesOrderService.getPriceOnProduct(formData.customer_id, Number(list?.new_qty), list?.product_id);
    ...
    const handleSODateChange = useCallback((dates: Date[]) => {
        setFormData((prev) => ({ ...prev, so_date: dates[0] ?? null }));
    ...
    const handleRemoveRow = async () => {
        ...
        let totalDeposit = 0;
        try {
            const deposit = await salesOrderService.getPaidAmounts(order_ids);
        ...
        switch (column) {
            case "product_code":
                try {
                    ...
                    setAgeType(ageType);
                    const res = await salesOrderService.getProductOnProductCode(value, formData.customer_id);
                ...
                break;
            case "customer_code":
                const res = await salesOrderService.getCustomerInfoByCode(value, "enter");
                ...
                if (res.list.existsSOID > 0) {
                    onChangeSalesOrderId(res.list.existsSOID);
                    onChangeSaveType("edit");
                    setShowCustomers(false);
                    return;
                }
    ...
    const handleVoid = async () => {
        const data = new FormData();
        data.append("details[]", JSON.stringify({ so_number: formData.so_number }));
        ...
        if (cancelType === "button") {
            ...
            try {
                ...
                if (result.token === "Error") {
                    ...
                    setLoadCancelOrder_ORDER(false);
                    showErrorToast(result.message);
                    return;
                }
    ...
    const handleRowClick_Customer = async (e: React.MouseEvent, tableId: number) => {
        ...
        const res = await salesOrderService.getCustomerInfoByCode(String(customer_code), "click");
        if (res.token === "CustomerNotExists") {
                        ...
                        <div className="col-span-12 p-1">
                            <fieldset className="grid grid-cols-12 gap-4 border-[1px] border-[#ffffff1a] rounded-lg p-4 mb-2">
                                <div className="col-span-12 md:col-span-4">
                                    ...
                                    <div className="flex items-center space-x-4 mb-2">
                                        <label className="w-[25%] text-gray-400 text-sm text-right">{translations["Customer Address"]}</label>
                                        <textarea
                                            rows={3}
                                            hidden={lang === "cn"}
                                            readOnly
                                ...
                                <div className="col-span-12 md:col-span-4">
                                    ...
                                    <div className="flex items-center space-x-4 mb-2">
                                        ...
                                        <textarea
                                            ...
                                            hidden={lang === "cn"}
                                            readOnly
                                            value={formData.delivery_address_en}
                                            onChange={(e) => {
                                                const value = e.target.value;
                                        ...
                                        <textarea
                                            ...
                                            hidden={lang === "en"}
                                            readOnly
                                            value={formData.delivery_address_cn}
                                            onChange={(e) => {
                                                const value = e.target.value;
                                ...
                                <div className="col-span-12 md:col-span-4">
                                    ...
                                    <div className="flex items-center space-x-4 mb-2">
                                        <label className="w-[25%] text-gray-400 text-sm text-right">{translations["SO Date"]}</label>
                                        <Flatpickr
                                            onChange={handleSODateChange}
                                            options={{
                                                dateFormat: "M d Y",
                                                defaultDate: formData.so_date || undefined,
                                                allowInput: true,
                                                locale: locale,
                                            }}
                                    ...
                                    <div className="flex items-center space-x-4 mb-2">
                                        <label className="w-[26%] text-gray-400 text-sm text-right">{translations["SO Type"]}</label>
                                        <Select
                                            classNamePrefix="react-select"
                        ...
                        <div className="grid grid-cols-12 gap-2">
                            <div className={`col-span-12 md:col-span-12`}>
                                <table className="w-full border border-gray-600 border-collapse text-sm text-left">
                                    <thead className="sticky top-0 z-[1] py-1 px-4" style={{ backgroundColor: "#1f2132" }}>
                                        <tr className="border-b" style={{ borderColor: "#2d2d30" }}>
                                            <th className="w-[35%] text-left py-2 px-2 text-gray-400 text-sm w-12 border border-[#40404042]">{translations["Product Code"]}</th>
                                            <th className="w-[65%] text-left py-2 px-2 text-gray-400 text-sm w-12 border border-[#40404042]">{translations["Product Name"]}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {selectedItems.length === 0 ? (
                                            ...
                                            selectedItems.map((item) => (
                                                <tr
                                                    ...
                                                    style={{ borderColor: "#40404042" }}
                                                >
                                                    <td className="py-2 px-2 text-gray-400 text-left text-custom-sm border border-[#40404042]">{item.product_code}</td>
                                                    <td className="py-2 px-2 text-gray-400 text-left text-custom-sm border border-[#40404042]">
                            ...
                            <div className={`col-span-12 md:col-span-12`}>
                                <button
                                    ...
                                    disabled={loadCancelOrder_CTOCUST || loadCancelOrder_NRTOMI || loadCancelOrder_RPTOC || loadCancelOrder_ORDER}
                                >
                            ...
                            {cancelType === "checkbox" && (
                                <div className={`col-span-12 md:col-span-12`}>
                                    <button
                                        onClick={() => handleSaveCancelOrder("PREORDER")}
                                        className={`px-2 py-2 bg-cyan-600 hover:bg-cyan-500 rounded text-white transition w-full
                                            ${loadCancelOrder_ORDER ? "bg-gray-500 cursor-not-allowed opacity-50" : "bg-cyan-600 hover:bg-cyan-700"} 
                                            text-[#ffffffcc] flex justify-center items-center`}
                                        disabled={loadCancelOrder_CTOCUST || loadCancelOrder_NRTOMI || loadCancelOrder_RPTOC || loadCancelOrder_ORDER}
                                    >
                                        {loadCancelOrder_ORDER ? (
                                            <>
                                                <div className="loader mr-2"></div>
                                                <span>{translations["Processing2"]}...</span>
                                            </>
                                        ) : (
                                            <span>{translations["Return to Preorder"]}</span>
                                        )}
                                    </button>
                                </div>
                            )}
                ...
                <div className="flex items-center justify-between px-2 py-3">
                    ...
                    <div className="flex items-center space-x-1">
                        ...
                        <button
                            ...
                            className={`px-2 py-2 text-custom-sm rounded-lg text-sm font-medium transition-colors 
                            ${loadingCreateInv ? "bg-gray-500 cursor-not-allowed opacity-50" : "bg-cyan-600 hover:bg-cyan-700"} 
                            text-[#ffffffcc] flex justify-center items-center`}
                            disabled={loadingCreateInv || salesOrderId === 0 || masterList?.invoice_status_id != 2}
                        >
                            {loadingCreateInv ? (
                                <>
                                    <div className="loader mr-2"></div>
                                ...
                                <span>{translations["Create Invoice"]}</span>
                            )}
                        </button>
                        <button
                            onClick={handleVoid}
                            disabled={salesOrderId === 0 || masterList?.invoice_status_id != 2}
