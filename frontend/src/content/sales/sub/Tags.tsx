...
const TagList: React.FC<TagListProps> = ({ tabId, onChangeView, fromView }) => {
    ...
    const [loading, setLoading] = useState(() => {
        const savedLoading = localStorage.getItem(`${tabId}-loading-tags`);
    ...
    const [currentPage, setCurrentPage] = useState(() => {
        const metaKey = `${tabId}-cached-meta-tags`;
    ...
    const [searchTerm, setSearchTerm] = useState(() => {
        const cached = localStorage.getItem(`${tabId}-cached-meta-tags`);
        if (!cached) return "";
        try {
    ...
    // ON LOAD LIST
    useEffect(() => {
        const productKey = `${tabId}-cached-mass-tags`;
        const metaKey = `${tabId}-cached-meta-tags`;
        const mountKey = `${tabId}-mount-status-tags`;
        const isFirstMount = sessionStorage.getItem(mountKey) !== "mounted";
        if (isFirstMount) {
            sessionStorage.setItem(mountKey, "mounted");
            fetchTagList(currentPage, itemsPerPage, searchTerm);
            return;
        }
        ...
        try {
            ...
            if (isCacheValid) {
                ...
                setLoading(false);
                localStorage.setItem(`${tabId}-loading-tags`, JSON.stringify(false));
                return;
            }
        } catch (err) {
            console.warn("Failed to parse cached meta:", err);
    ...
    const fetchTagList = async (page = currentPage, perPage = itemsPerPage, search = "") => {
        try {
            if (loading) {
                setLoading(true);
            }
            localStorage.setItem(`${tabId}-loading-tags`, JSON.stringify(true));
            const paginatedData = await massMailerService.getTags(page, perPage, search);
            setProducts(paginatedData.data);
            setTotalPages(paginatedData.last_page);
            localStorage.setItem(`${tabId}-cached-mass-tags`, JSON.stringify(paginatedData.data));
            localStorage.setItem(
                `${tabId}-cached-meta-tags`,
                JSON.stringify({
                    page,
                    perPage,
                    search,
    ...
    const handleSelectSingle = (preorderId: number, checked: boolean) => {
        if (checked) {
            setSelectSettings([...selectedSettings, preorderId]);
        } else {
            setSelectSettings(selectedSettings.filter((id) => id !== preorderId));
        }
    };
    const handleSavePopup = async () => {
        const data = new FormData();
        const email_address = formData["email_address"]?.toString().trim() ?? "";
    ...
    const handleDeleteSelected = async () => {
        ...
        if (!confirmed) return;
        try {
            const res = await massMailerService.deleteTags(selectedSettings);
            if (res.token === "Error") {
                ...
                <div className="p-4 border-b flex-shrink-0" style={{ borderColor: "#404040" }}>
                    <div className="flex justify-between items-center">
                        <div className="flex items-center space-x-4">
                            ...
                            <button
                                onClick={() => onChangeView(fromView !== "details" ? "list" : "details", "tags")}
                            ...
                            <button
                                onClick={() => onChangeView("settings", "tags")}
                            ...
                            <button
                                onClick={() => onChangeView("template", "tags")}
                                className="ml-2 px-2 py-2 border rounded-lg text-white transition-colors flex items-center space-x-px hover:bg-gray-600 text-sm"
                                style={{ backgroundColor: "#2d2d30", borderColor: "#2d2d30", marginLeft: "5px" }}
                            >
                                            ...
                                            <React.Fragment key={list.id || index}>
                                                ...
                                                >
                                                    <td className="py-2 px-2" onClick={(e) => e.stopPropagation()}>
                                                        <CustomCheckbox checked={selectedSettings.includes(list.id as number)} onChange={(checked) => handleSelectSingle(list.id as number, checked)} />
                                                    </td>
